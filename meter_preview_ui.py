...
<body>
                    ...
                    <div class="camera-controls">
                        <div class="control-section">
                            ...
                            <div class="preset-buttons">
                                <button class="btn-preset" onclick="applyPreset('{{ meter.type }}', 'night_vision')">🌙 Night Vision</button>
                                <button class="btn-preset" onclick="applyPreset('{{ meter.type }}', 'day_clear')">☀️ Day Mode</button>
                                <button class="btn-preset" onclick="applyPreset('{{ meter.type }}', 'low_noise')">🔇 Low Noise</button>
                                <button class="btn-preset" onclick="applyPreset('{{ meter.type }}', 'high_detail')">🔍 High Detail</button>
                                <button class="btn-preset" onclick="applyPreset('{{ meter.type }}', 'balanced')">⚖️ Balanced</button>
                                <button class="btn-preset" onclick="applyPreset('{{ meter.type }}', 'auto_adaptive')">🤖 Auto</button>
                            </div>
                        </div>
                        ...
                        <div class="control-section">
                            ...
                            <div class="action-buttons">
                                <button class="btn-action btn-optimize" onclick="runOptimization('{{ meter.type }}')">
                                    🔬 Auto-Optimize (~12 min)
                                </button>
                                <button class="btn-action btn-reading" onclick="triggerReading('{{ meter.type }}')">
                                    📸 Take Reading Now
                                </button>
                            </div>
                        </div>
    ...
    <script>
        ...
        async function applyPreset(meterType, presetName) {
            ...
            try {
                ...
                if (data.status === 'success') {
                    showStatus(meterType, '✓ ' + data.message + '. Wait 3 seconds then take a reading.', 'success');
                    ...
                    showStatus(meterType, '✗ Failed to apply preset: ' + data.message, 'error');
                }
            } catch (error) {
        ...
        async function runOptimization(meterType) {
            ...
            try {
                ...
                if (data.status === 'started') {
                    showStatus(meterType,
                        '🔬 Optimization running in background. Check logs/optimization_results.json in ~12 minutes.',
                        'processing'
                    );
                } else {
                    showStatus(meterType, '✗ Failed to start optimization: ' + data.message, 'error');
                }
            } catch (error) {
                showStatus(meterType, '✗ Error: ' + error.message, 'error');
            }
        ...
        async function triggerReading(meterType) {
            ...
            try {
                ...
                if (data.status === 'triggered') {
                    showStatus(meterType, '📸 Reading in progress... Refresh in 30 seconds to see results.', 'info');
                    ...
                    showStatus(meterType, '✗ Failed to trigger reading', 'error');
                }
            } catch (error) {
                showStatus(meterType, '✗ Error: ' + error.message, 'error');
            }
